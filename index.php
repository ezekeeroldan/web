...
$allorders = $conn->prepare("SELECT * FROM orders WHERE ORDER BY order_id DESC LIMIT 1  ");
 ...
 $unpaidCount =  countRow(["table"=>"invoices","where"=>["invoice_status"=>"Unpaid" , "client_id" => $user["client_id"]]]);
	...
	if( $_SESSION["msmbilisim_userlogin"] != 1  || $user["client_type"] == 1  ){
      ...
      array(
            ...
            'footerCode'=>$settings["custom_footer"],'lang'=>$languageArray,'timezones'=>$timezones,'AllOrders'=>$panel_orders, 'panel'=>$panel, 'general'=>$general, 'updates'=>$updates , 'blogs'=>$blogs, 'blogtitle'=>$blogtitle, 'blogcontent'=>$blogcontent, 'blogimage'=>$blogimage, 'currency'=>$currency, 'currencies'=>$currencies, 'menus'=>$menus, 'MenuSlug'=>$menuslug, 'pageurl'=>$pageurl , 'pagekeywords'=>$seo_keywords, 'pagetitle'=>$seo_title, 'pagedescription'=>$seo_description, 'PageText'=>$PageText , "news"=>$news,"totalorders"=>$totalorder
      )
    );
	 ...
	 echo $twig->render( $templateDir.'.twig',
      array(
            'site'=>[
                      ...
                      "description"=>$settings["site_description"],"allorders"=>$allorders,'languages'=>$languagesL,"refillavailable"=>$refillavailable,"dripfeedcount"=>$dripfeedcount,"subscriptionscount"=>$subscriptionscount,"childpanel_price"=>$settings["childpanel_price"],"inr_symbol"=>$settings["inr_symbol"],"inr_value"=>$settings["inr_value"],"ns1"=>$settings["ns1"],"ns2"=>$settings["ns2"],"snow_colour"=>$settings["snow_colour"],"snow_effect"=>$settings["snow_effect"],"popup"=>$settings["popup"]  ],
            'styleList'=>$stylesheet["stylesheets"],'scriptList'=>$stylesheet["scripts"],'captchaKey'=>$settings["recaptcha_key"],'captcha'=>$captcha,'resetPage'=>$resetPage,'serviceCategory'=>$categories,'categories'=>$categories,
            'error'=>$error,'errorText'=>$errorText,'success'=>$success,"servicesPage"=>$serviceList,"resetType"=>$resetType,'successText'=>$successText,'title'=>$title,"Signup"=>$Signup,"ChildPanel"=>$ChildPanel,'affiliatessystem'=>$affiliatessystem,'Promotion'=>$Promotion,
        ...
        'popupNotif' => $popupNotif , 'ref_content' => $ref_content , 'ref_payouts' => $ref_payouts , 'panels' => $panels , 'invoices' => $invoices , "route" => $routes , "unpaidCount" => $unpaidCount , 'panel'=>$panel, 'general'=>$general, 'updates'=>$updates , 'blogs'=>$blogs, 'blogtitle'=>$blogtitle, 'blogcontent'=>$blogcontent, 'blogimage'=>$blogimage, 'currency'=>$currency, 'currencies'=>$currencies, 'menus'=>$menus, 'MenuSlug'=>$menuslug, 'pageurl'=>$pageurl  , 'pagekeywords'=>$seo_keywords, 'pagetitle'=>$seo_title, 'pagedescription'=>$seo_description , 'PageText'=>$PageText, "news"=>$news,"totalorders"=>$totalorder,'AllOrders'=>$panel_orders
      ) 
